...
app = Flask(__name__)
app.secret_key = 'BxM8Q~Luz5txq1sXkhFsVaEai9yH11sjtp1llacV'
...
# config = {
#   'host':'group13.mysql.database.azure.com',
#   'user':'group13',
#   'password':'myFirst@12',
#   'database':'group13',
#   'port':3306,
#   'ssl_ca': 'C:/Users/saish/Downloads/DigiCertGlobalRootCA.crt.pem',
#   'connect_timeout':50000
# }
...
def upload():
    ...
    if request.method == 'POST':
        ...
        if hdata.filename == '' or tdata.filename == '' or pdata.filename == '' :
            ...
            col_names=['BASKET_NUM','HSHD_NUM','PURCHASE_DATE','PRODUCT_NUM','SPEND','UNITS','STORE_R','WEEK_NUM','YEAR']
            ...
            for i,row in csvData.iterrows():
                query += '{},'.format((row['BASKET_NUM'],row['PURCHASE_DATE'],row['SPEND'],row['UNITS'],row['STORE_R'],row['WEEK_NUM'],row['YEAR'],row['HSHD_NUM'],row['PRODUCT_NUM']))
            query = query[:len(query)-1]
            cur.execute(query)
